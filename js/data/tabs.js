...
const TABS = [
    ...
    {title: "Statistics"},
    {title: "Achievements"},
    {title: "Infinity", style: "inf", unl() { return player.inf.infinitied.gte(1) }, over: `
        <span class="txt">You have <span id="infAmount">X</span> Infinity points.</span><br>
    `},
]
...
el.setup.tabs = _=>{
    ...
    for (let x = 0; x < TABS.length; x++) {
        inner += `
        <button onclick="changeTab(${x})" id="tab_${x}" class="tab ${TABS[x].style||""}">${TABS[x].title}</button>
        `
        ...
        if (TABS[x].group) {
            ...
            <button onclick="changeTab(${y}, true)" id="stab_${x}_${y}" class="tab stab ${TABS[x].group[y].style||""}">${TABS[x].group[y].title}</button>
            `
            inner2 += "</div>"
        }
    }
